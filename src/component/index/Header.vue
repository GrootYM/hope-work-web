...
<template>
  <div>
    ...
    <div id="headTop">
      <div class="contains">
        <div class="left">
          <img src="@/assets/image/about/logo-2024-2.jpg" class="home-icon" />
          <div class="location" @click="citySelect" v-if="$route.path === '/index/home'">
        ...
        <ul class="right1">
          ...
          <li :class="{ nowAt: activeRouter === 7 }" v-if="isUser" class="unreadLi">
            <router-link to="/index/immediately">
              通知
              <span class="unreadSpan" v-show="$route.name != 'immediately' && unreadSum && unreadSum > 0">{{
                unreadSum
              }}</span>
            </router-link>
          </li>
          <li>
            <el-dropdown v-if="isUser">
              <div class="avatar">
                <div class="user-name">{{ userInfo.userName || userInfo.mobile }}</div>
                <img :src="info.headUrl || info.headPath" />
              </div>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item @click.native="gotoUserCenter">个人中心</el-dropdown-item>
                <el-dropdown-item @click.native="gotoShieldCompany">隐私保护</el-dropdown-item>
    ...
    <el-dialog :visible.sync="cityShow" :show-close="false">
	  ...
	  <div class="tabsCity">
	      <ul class="navlist">
	        <li :class="{ active: isDomestic }" @click="switchTab(true)">国内</li>
	        <li :class="{ active: !isDomestic }" @click="switchTab(false)" style="margin-right: auto">国外</li>
	      </ul>
	    </div>
...
export default {
  ...
  methods: {
	  ...
	  // fetchCityData() {
	  //   if (this.isDomestic) {
	  //     this.isOverseasCity = 0;
	      
	  //   } else {
	  //     this.isOverseasCity = 1;
	  //   }
	  // },
    gotoShieldCompany() {
    ...
    async queryCityList() {
	  ...
	  let res ={code:404}
	  if(this.isDomestic){
		 res = await Job.queryCityList(params) 
	  }else{
		 res = await Job.queryOverseasCityList(params.regionLevel,params.id)
	  }
      if (res.code == 200) {
        this.provinceArr = res.data
		if(res.data && res.data[0]){
			this.changeProvince(res.data[0].id)
		}
        res.data.forEach(p => {
    ...
    changeProvince(val) {
      ...
      this.provinceArr.forEach(item => {
        if (val == item.id) {
          ...
          this.$store.commit('SET_NOW_PROVINCE', { province: item.name, zipcode: item.adcode })
        }
      })
      this.queryCityList1()
    },
